...
import { ConfigureVideosComponent } from './configure-videos.component';

describe('ConfigureVideosComponent', () => {
  let component: ConfigureVideosComponent;
  var videoPlaylist: VideoResponse[] = [];
  var tempPath: string;
  var thumbnailData: string[];
  var teamId: string;
  var tempThumbnailPath: string;
  let fixture: ComponentFixture<ConfigureVideosComponent>;
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      declarations: [ConfigureVideosComponent],
      providers: [
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(ConfigureVideosComponent);
  ...
  it('delete video should give error for null data', () => {
    component.deleteVideo(null).catch((e) => {
      expect(e).toEqual('Cannot DELETE /v1/videos/delete/');
    });
  });

  it('video playlist not to be null', () => {
    thumbnailData = [];
    videoPlaylist = JSON.parse(localStorage.getItem('TeamDetailsResponse'))
      .powerboardResponse.videos;
    teamId = JSON.parse(localStorage.getItem('TeamDetailsResponse'))
      .powerboardResponse.team_id;
    for (let video of videoPlaylist) {
      tempPath =
        'assets/uploads/multimedia/' + teamId + '/videos/' + video.videoPath;

      tempThumbnailPath = tempPath
        .replace('videos', 'thumbnails')
        .replace('.mp4', '.png');
      thumbnailData.push(tempThumbnailPath);
    }
    component.updateComponent();
    expect(component.videoPlaylist).toEqual(videoPlaylist);
  });
});
