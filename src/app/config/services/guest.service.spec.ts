...
describe('GuestService', () => {
  ...
  it('should throw bad request for null values', async () => {
    ...
    let req = httpTestingController.expectOne(
      environment.restPathRoot + 'v1/auth/add-guest'
    );
    req.flush('Bad Request', {
      status: 400,
      statusText:
        'User is not guest, Try diffrent method to register diffrent users',
    });
  });
  ...
  it('should add and delete Guest', async () => {
    ...
    service
      ...
      .then((data) => {
        ...
        service
          .deleteGuest(result.id)
          .then((deleteData) => {
            httpTestingController.expectOne(
              environment.restPathRoot + 'v1/admin/delete/guest/' + result.id
            );
          })
          .catch((error) => {
            result = error;
