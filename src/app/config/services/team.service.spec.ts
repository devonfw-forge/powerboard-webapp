...
describe('TeamService', () => {
  ...
  it('view all team members should throw error for null ', async () => {
    ...
    let req = httpTestingController.expectOne(
      environment.restPathRoot + 'v1/admin/viewAllMemberOfTeam/null'
    );
    req.flush('Internal Server Error', {
      status: 500,
      statusText: 'Something went wrong, Please try again in some moment',
  ...
  it('change team member role should get error for null', () => {
    ...
    let req = httpTestingController.expectOne(
      environment.restPathRoot + 'v1/admin/update/userRole'
    );
    req.flush('Internal Server Error', {
      status: 500,
      statusText: 'Something went wrong, Please try again in some moment',
  ...
  it('should add and delete team member', async () => {
    ...
    service
      ...
      .then((data) => {
        ...
        service
          .deleteTeamMember(result.id)
          .then((deleteData) => {
            httpTestingController.expectOne(
              environment.restPathRoot + 'v1/admin/delete/userTeam/' + result.id
            );
          })
          .catch((error) => {
            result = error;
  ...
  it('add team should throw internal server error', async () => {
    ...
    let req = httpTestingController.expectOne(
      environment.restPathRoot + 'v1/admin/team/addTeam'
    );
    req.flush('Internal Server Error', {
      status: 500,
      statusText: 'Something went wrong, Please try again in some moment',
