...
import { CodeQualityResponse } from 'src/app/model/general.model';
...
describe('CodeQualityComponent', () => {
  ...
  it('should have precise data',inject([HttpClient], async (http:HttpClient) => {
    let codeQuality : CodeQualityResponse = {
      "bugs": 3,
      "codeSmells": 13,
      "codeCoverage": 85,
      "status": "PASSED"
   ...
   let data :any = JSON.parse(localStorage.getItem('TeamDetailsResponse')).powerboardResponse.dashboard.codeQuality;
   component.codeQuality= data;
   
    fixture.detectChanges();
 expect(component.codeQuality).toEqual(codeQuality);
   })); 
});
