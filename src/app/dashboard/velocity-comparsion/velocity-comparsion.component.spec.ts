...
import { Dashboard, PowerboardResponse, TeamDetailResponse, VelocityResponse } from 'src/app/model/general.model';

import { VelocityComparsionComponent } from './velocity-comparsion.component';

describe('VelocityComparsionComponent', () => {
  let component: VelocityComparsionComponent;
  let fixture: ComponentFixture<VelocityComparsionComponent>;
  let teamResponse = {
    "powerboardResponse": {
  ...
  it('should have precise data',inject([HttpClient], async (http:HttpClient) => {
    let velocity : VelocityResponse = {
      "Avg": 115,
      "Committed": 140,
      "Completed": 18
  }
    
   let data :any = JSON.parse(localStorage.getItem('TeamDetailsResponse')).powerboardResponse.dashboard.velocity; ;
